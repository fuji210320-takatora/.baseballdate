...
# 異体字・旧字体を標準化するための変換辞書
KANJI_MAP = str.maketrans('澤髙﨑嵜嶋濱邊邉廣櫻眞萬齋齊', '沢高崎崎島浜辺辺広桜真万斎斎')
...
def load_data():
        ...
        try:
            ...
            if not df_pos.empty:
                if not any("選手" in str(c) for c in df_pos.columns):
                    for i, row in df_pos.head(10).iterrows(): 
                ...
                if name_col:
                    ...
                    loaded_positions.append(pos_name) 
    ...
    if not df_defense_all.empty and "選手名" in df_defense_all.columns:
        df_defense_all["選手名"] = df_defense_all["選手名"].astype(str).str.replace(r'\s+', '', regex=True).str.translate(KANJI_MAP)
